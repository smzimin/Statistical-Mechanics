    ...
    while True:
        ...
        if step % N == 0:
            ...
            if n_diff == 0:
                ...
                #break
        #if step % (10 * N) == 0:
        #    show_spins(S0, S1, L, str(step / N) + '_' + str(L) + '_' + str(T))
...
nruns = 10
avrgTimes = []
temp = [5.0, 4.0, 3.0, 2.5, 2.4]

for T in temp: #
    avrgTime = 0
    for _ in range(nruns):
        avrgTime += mainC2()
    print(T, avrgTime / nruns)
    avrgTimes.append(avrgTime / nruns)

pylab.plot(temp, avrgTimes, 'ro')
pylab.plot(temp, avrgTimes, 'r', linewidth = 3)
