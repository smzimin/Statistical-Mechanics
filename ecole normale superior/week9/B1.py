import random, math, multiprocessing as mp
...
def mainB1(run, sigma):
    ...
    while sigma > 1.e-8:
        ...
        list_without_k = [l for l in range(N)]
        list_without_k.remove(k)
        ...
        new_min_dist = min([math.sqrt(sum((positions[l][j] - newpos[j]) ** 2 for j in range(3))) for l in list_without_k])
